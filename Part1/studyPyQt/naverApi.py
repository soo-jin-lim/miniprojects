# NaverApi 클래스 - OpenAPI 인터넷 통해서 데이터를 전달 받음 l9Bqmh3hDnZMXOxNu4cF ATBI1wc5Ha
...
class NaverApi:
    ...
    def get_request_url(self, url):
        req = Request(url)
        # Naver API 개인별 인증
        req.add_header('X-Naver-Client-Id', 'l9Bqmh3hDnZMXOxNu4cF')
        req.add_header('X-Naver-Client-Secret', 'ATBI1wc5Ha')
    ...
    def get_post_data(self, post, outputs):
        title = post['descripsion']
        originallink = post['originallink']
        link = post['link']

        pubDate = datetime.datetime.strptime(post['pubDate'],'%a, %b %b %Y %H: %M:%S +9000')
